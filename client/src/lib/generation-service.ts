...
export async function generateGradeForPart(input: { partKey: PartKey; topic: string; script: ScriptArtifact; config?: AiConfig; provider?: AiProvider; signal?: AbortSignal }): Promise<GradeArtifact> {
  ...
  const relayAbort = () => controller.abort(); input.signal?.addEventListener('abort', relayAbort, { once: true })
  try {
    const raw = await provider.complete({ baseUrl: config.baseUrl, model: config.model, temperature: config.temperature, maxTokens: config.maxTokens, apiKey: config.apiKey || undefined, messages: [{ role: 'system', content: `Return ONLY valid JSON for a color grade with keys filter, intensity, exposure, contrast, saturation, temperature, notes. Keep numbers within sensible CapCut ranges. Write any notes in ${config.scriptLanguage}.` }, { role: 'user', content: `Topic: ${input.topic}\nPart: ${input.partKey}\nScript: ${input.script.markdown}` }] }, controller.signal)
