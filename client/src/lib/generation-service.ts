...
export async function generateFieldsForPart(input: { partKey: PartKey; topic: string; notes: string; rulesVersion: string; config?: AiConfig; provider?: AiProvider; signal?: AbortSignal }) {
  ...
  try {
    const prompt = buildFieldsPrompt(input)
    const raw = await provider.complete({ baseUrl: config.baseUrl, model: config.model, temperature: config.temperature, maxTokens: config.maxTokens, apiKey: config.apiKey || undefined, messages: [{ role: 'system', content: prompt.system }, { role: 'user', content: prompt.user }] }, controller.signal)
...
export async function generateScriptForPart(input: { partKey: PartKey; topic: string; notes: string; fields: FieldSet; config?: AiConfig; provider?: AiProvider; signal?: AbortSignal }): Promise<ScriptArtifact> {
  ...
  try {
    const prompt = { system: 'You are an Egyptian Arabic technical video script editor. Return only Markdown, with headings Hook, Story, Technical Breakdown, Proof, Verdict, and CTA. Do not invent factual specifics absent from the grounding notes.', user: `Topic: ${input.topic}\nGrounding notes: ${input.notes || '(none)'}\nFields: ${JSON.stringify(input.fields)}\nPart: ${input.partKey}` }
    const markdown = await provider.complete({ baseUrl: config.baseUrl, model: config.model, temperature: config.temperature, maxTokens: config.maxTokens, apiKey: config.apiKey || undefined, messages: [{ role: 'system', content: prompt.system }, { role: 'user', content: prompt.user }] }, controller.signal)
...
export async function generateMontageForPart(input: { partKey: PartKey; topic: string; script: ScriptArtifact; config?: AiConfig; provider?: AiProvider; signal?: AbortSignal }): Promise<{ shots: MontageShot[]; warnings: string[] }> {
  ...
  const relayAbort = () => controller.abort(); input.signal?.addEventListener('abort', relayAbort, { once: true })
  try {
    const raw = await provider.complete({ baseUrl: config.baseUrl, model: config.model, temperature: config.temperature, maxTokens: config.maxTokens, apiKey: config.apiKey || undefined, messages: [{ role: 'system', content: 'Return ONLY valid JSON in the shape {"shots":[{"tStart":"0:00","tEnd":"0:10","shot":"talking-head","camera":"medium-static","onScreen":"","note":""}]}. Timecodes must be ordered and shot names must be concise.' }, { role: 'user', content: `Topic: ${input.topic}\nPart: ${input.partKey}\nScript: ${input.script.markdown}` }] }, controller.signal)
...
export async function generateGradeForPart(input: { partKey: PartKey; topic: string; script: ScriptArtifact; config?: AiConfig; provider?: AiProvider; signal?: AbortSignal }): Promise<GradeArtifact> {
  ...
  const relayAbort = () => controller.abort(); input.signal?.addEventListener('abort', relayAbort, { once: true })
  try {
    const raw = await provider.complete({ baseUrl: config.baseUrl, model: config.model, temperature: config.temperature, maxTokens: config.maxTokens, apiKey: config.apiKey || undefined, messages: [{ role: 'system', content: 'Return ONLY valid JSON for a color grade with keys filter, intensity, exposure, contrast, saturation, temperature, notes. Keep numbers within sensible CapCut ranges.' }, { role: 'user', content: `Topic: ${input.topic}\nPart: ${input.partKey}\nScript: ${input.script.markdown}` }] }, controller.signal)
