...
export function migrateLegacyPacks(): { migrated: number; packs: CombinedPack[] } {
  ...
  try {
    ...
    const migrated = legacy.filter(Boolean).map((item, index) => ({ schemaVersion: 2 as const, packVersion: 1 as const, meta: { id: `legacy-${item.meta?.createdAt || index}`, topic: item.meta?.topic || '', notes: item.meta?.notes || '', createdAt: item.meta?.createdAt || new Date().toISOString(), updatedAt: new Date().toISOString(), rulesVersion: 'v3.2', model: 'Legacy import' }, parts: [{ key: 'long' as const, label: 'Long-form', stageStatus: { fields: 'done' as const, script: 'idle' as const, montage: 'idle' as const, grade: 'idle' as const }, fields: { title: item.fields?.title || '', promise: item.fields?.promise || '', audience: '', hook: '', story: '', insight: '', proof: '', payoff: '', cta: '' } }, ...(['short-1', 'short-2', 'short-3', 'short-4', 'short-5'] as const).map((key) => ({ key, label: `Short ${key.replace('short-', '#')}`, stageStatus: { fields: 'idle' as const, script: 'idle' as const, montage: 'idle' as const, grade: 'idle' as const } }))], runSheet: [] }))
