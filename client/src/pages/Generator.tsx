...
export default function Generator() {
  ...
  const packVersions = trpc.workspace.listPackVersions.useQuery({ workspaceId: workspaceId || 'pending', projectId: projectId || 'pending' }, { enabled: Boolean(workspaceId && projectId), retry: false })
  const generationRuns = trpc.workspace.listGenerationRuns.useQuery({ workspaceId: workspaceId || 'pending', projectId: projectId || 'pending' }, { enabled: Boolean(workspaceId && projectId), retry: false })
  ...
  useEffect(() => {
    if (!run || run.status === 'idle' || !workspaceId || !projectId) return
  ...
  }, [projectId, run, saveCloudRun, trpcUtils.workspace.listGenerationRuns, workspaceId])

  useEffect(() => {
    if (!workspaceId || !projectId || isRunning || run || !generationRuns.data) return
  ...
  }, [generationRuns.data, isRunning, projectId, recover, run, workspaceId])
  ...
  return <div className="page page-generator">
    ...
    <ProjectHistoryPanel cloudActive={Boolean(workspaceId && projectId)} versions={packVersions.data || []} runs={generationRuns.data || []} loading={packVersions.isLoading || generationRuns.isLoading} restoringVersionId={restoreCloudVersion.isPending ? restoreCloudVersion.variables?.versionId || null : null} onRestore={handleRestoreVersion} />
