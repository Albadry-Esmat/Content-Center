  ...
  return <div className="page page-settings settings-page">
    ...
    <div className="settings-grid">
      ...
      <section id="settings-provider" className="settings-card">
        ...
        <div className="provider-panel-overview"><div><b>Connect the AI you want to use.</b><p>Start locally for private drafting, or use a hosted provider through the protected server proxy.</p></div><span className="provider-route-badge">{config.providerMode === 'local' ? 'LOCAL' : 'HOSTED'}</span></div>
        <div className="provider-route-summary" role="status"><ShieldCheck size={16} /><span><b>{provider.label}</b><small>{provider.requiresServerProxy ? 'Protected server route · browser credential not used' : 'Browser-to-local route · prompts stay on the configured machine'}</small></span></div>
        ...
        <div className="provider-field-group"><span className="provider-group-label">Provider details</span>
          ...
          {config.providerMode === 'local' && <div className="provider-field"><label htmlFor="base-url">OpenAI-compatible base URL</label><input id="base-url" value={config.baseUrl} onChange={(event) => update('baseUrl', event.target.value)} /><p className="field-hint">Use the URL shown by your local model server.</p></div>}
          <div className="provider-field"><label htmlFor="model">Model ID</label><input id="model" list="model-options" value={config.model} onChange={(event) => update('model', event.target.value)} /><datalist id="model-options">{modelOptions.map((model) => <option key={model} value={model} />)}</datalist><p className="field-hint">Use an exact ID returned by Discover models.</p></div>
        </div>
        <div className="provider-action-panel"><div><b>Check before generating</b><small>Discover reads available model IDs. Test confirms the selected route and model.</small></div><div className="connection-actions"><button className="button button-quiet" onClick={testConnection} disabled={testing}>{testing ? <><Loader2 className="spin" size={15} /> Testing connection…</> : <><CircleCheck size={15} /> Test connection</>}</button><button className="button button-quiet" onClick={discoverModels} disabled={discoveringModels}>{discoveringModels ? <><Loader2 className="spin" size={15} /> Discovering models…</> : <><SlidersHorizontal size={15} /> Discover models</>}</button></div></div>
        <details className="provider-diagnostics" open={Boolean(modelDiscoveryNotice || connectionResult || discoveredModels.length)}>
          <summary><span><b>Connection & model diagnostics</b><small>{modelDiscoveryNotice || connectionResult ? 'Latest result available' : 'Results appear after Test connection or Discover models'}</small></span><span className="provider-diagnostics-toggle">View details</span></summary>
          <div className="provider-diagnostics-content">
        ...
        <div id="settings-privacy" className="privacy-panel"><div className="privacy-panel-heading"><span><span className="section-index">PRIVACY & RECOVERY</span><b>Keep the trust boundary clear.</b></span><ShieldCheck size={17} /></div><p><strong>This browser stores preferences only.</strong> Hosted provider credentials stay on the server, never in browser settings.</p><details className="privacy-details"><summary>What can leave this browser?</summary><ul><li>Local mode sends prompts to the local endpoint you choose.</li><li>Hosted mode sends prompts through the protected provider proxy.</li><li>Support evidence should exclude keys, cookies, private prompts, and personal notes.</li></ul></details></div>
      </section>
    </div>
