...
Every successful draft carries the existing generation provenance shape with `source: ai`, provider ID, provider mode, model, and generation timestamp. This provenance is descriptive metadata for review and recovery; it does not certify the truth of any claim. The visible generation action is explicit, and the current review workspace lets the creator edit each section before choosing one of four decisions: keep existing notes, append the draft to notes, replace notes explicitly, or discard the draft. Existing notes are never silently changed. The review workspace shows a persistent warning that AI planning text is not verified research, surfaces verification placeholders and open questions, and marks edited drafts as requiring another review. Persistence, downstream acceptance, and staleness handling remain gated behind later batches.
