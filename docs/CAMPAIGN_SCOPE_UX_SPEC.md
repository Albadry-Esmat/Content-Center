...
Every successful draft carries the existing generation provenance shape with `source: ai`, provider ID, provider mode, model, and generation timestamp. This provenance is descriptive metadata for review and recovery; it does not certify the truth of any claim. The visible generation action is explicit, and the current review workspace lets the creator edit each section before choosing one of four decisions: keep existing notes, append the draft to notes, replace notes explicitly, or discard the draft. Existing notes are never silently changed. The review workspace shows a persistent warning that AI planning text is not verified research, surfaces verification placeholders and open questions, and marks edited drafts as requiring another review. Accepted append/replace decisions are tracked against a topic-plus-notes snapshot. If the topic or notes change afterward, the handoff is marked stale and downstream generation remains available with a warning that it uses the current notes rather than an accepted AI foundation. Batch 7 persists the foundation draft, warnings, decision, accepted snapshot, and sanitized provenance in a versioned local foundation session keyed by pack ID. Backup export/import carries those sessions alongside packs, legacy pack-only arrays remain valid, unsupported provider metadata is discarded during normalization, and API keys, endpoint URLs, and authorization values are never stored in the session. Deleting a saved pack also deletes its foundation session. Review hardening adds useful empty-state examples, an explicit provider-not-connected state, sanitized retry recovery, real multiline list editing and note separators, and direction-aware foundation fields. Cloud persistence remains gated behind later batches.
...
| FR-07 | Editable review, explicit keep/append/replace/discard decisions, and grounding warnings |
| FR-08 | Accepted foundation handoff, topic-plus-notes snapshot, stale-state recovery, and downstream warning |
| FR-09 | Local foundation-session persistence, sanitized backup/restore, legacy compatibility, and deletion cleanup |
